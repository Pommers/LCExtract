...
Module
------
seeing.py: Wrapper for image retrieval module
...
from LCExtract.utilities import threeSigma, gaussian
from LCExtract.ztf import refZTFobj, getZTFOidOffsetStats


def startup():
    print()
    print('Seeing analysis retrieval module\n'
          '----------------------')
    print()
...
def lcHistogram(LCData):
    if not os.path.isdir(config.plotsPath):
        os.makedirs(config.plotsPath)

    title = f'{LCData.AO.objectName} - Lightcurve magnitude distribution'
    fig = plt.figure(figsize=(11, 8), dpi=300)
    fig.suptitle(title, size=18)
    for f in LCData.filtersReturned:
        table = LCData.table
        table = table[table['filterID'] == f]
        bins = 25  # int(len(table) / 10)
        # bins = int(LCData.samples[f]/10)
        table['mag'].plot.hist(bins=bins, color=f, alpha=0.5, density=False, label=f'{f}-mag')
        # plt.axvline(x=LCData.mean[f], color='black', alpha=0.8)
        mag_interval = 0.01
        mag_range = np.arange(min(table['mag']), max(table['mag']), mag_interval)
        SD = threeSigma('z', f, LCData.mean[f]) / 3
        norm = gaussian(LCData.mean[f], SD)
        bin_density = len(table) * ((max(table['mag']) - min(table['mag'])) / bins)
        plt.plot(mag_range, norm.pdf(mag_range) * bin_density, color='gray')
        # norm_act = gaussian(LCData.mean[f], LCData.SD[f])
        # plt.plot(mag_range, norm_act.pdf(mag_range)*bin_density, color='black')
        for s in range(-3, 4, 1):
            smag = LCData.mean[f] + (SD * s)
            if smag < min(table['mag']) or smag > max(table['mag']):
                continue
            sfreq = norm.pdf(smag) * bin_density
            alpha = 1 - (0.2 * abs(s))
            plt.vlines(x=smag, color='black', alpha=alpha, ymin=0, ymax=sfreq)
            # plt.axvline(x=LCData.mean[f]-(SD*s), color='black', alpha=0.8-(0.1*s), ymax=sfreq)
    plt.title(f'25 bins / filter with {chr(963)} intervals', size=14)
    plt.xlabel('Detection magniude (mag)', size=12)
    plt.ylabel('Detection count', size=12)
    plt.legend()
    plt.show()
    plt.savefig(config.ZTFScFilename)
